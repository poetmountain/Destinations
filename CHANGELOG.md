...
* Added an `AutoCaseIterable` macro to enable automatic conformance to `CaseIterable` to any enums you apply it to, even when an enum has cases with associated values. `RoutableDestinations` and `EventTypeable` protocols now require `CaseIterable` conformance, so adoption should be easier and eliminate the need to manually update an array of enum cases.
